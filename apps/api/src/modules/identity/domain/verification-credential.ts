...
const verificationSecretPattern = /^[A-Za-z0-9_-]{43}$/;
...
export function parseVerificationCredential(input: string): VerificationCredential {
  const separatorIndex = input.indexOf(".");
  const hasOneSeparator = separatorIndex > 0 && separatorIndex === input.lastIndexOf(".");
  if (!hasOneSeparator) {
    throw new IdentityInputValidationError("token", "VERIFICATION_TOKEN_INVALID");
  }

  const tokenId = input.slice(0, separatorIndex);
  const secret = input.slice(separatorIndex + 1);
  if (!tokenIdSchema.safeParse(tokenId).success || !verificationSecretPattern.test(secret)) {
    throw new IdentityInputValidationError("token", "VERIFICATION_TOKEN_INVALID");
  }
